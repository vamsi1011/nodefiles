...
function endsWithUrl(url) { 
    ...
    if(url.split('/').length<5){
        if(!(url.slice(url.lastIndexOf('/'),-1).includes('=') || 
            ...
            console.log("single slash : 1st "+ url);
            return true;
            }
    }
        ...
        if(url.slice(url.lastIndexOf('/'),-1).length==0){
            ...
            if(!(urlParse.path.includes('=') || urlParse.path.includes('-'))){
                console.log("single slash : 2st "+ url);
                return true;
                }
        }
    }  
...
async function process1(start, params){
    ...
    careerLink=params.joburl;
        ...
        let indexSelector = 0;
        if (totalJobsCount > 60) {
            indexSelector = parseInt(totalJobsCount / 60);
            if (indexSelector <= 1) {
                indexSelector = 2
            }
        }
        ...
        let pageCheckJob = [],pageCheckNoJob=[]
        await Promise.all(
    ...
    let mostCommonString= await mostCommonSubstring(pageCheckJob);
    console.log("jobs array")
    console.log(jobs);
    console.log("jobs array mm........")
    ...
    return {"HCS":mostCommonString, "jobUrls":pageCheckJob, "perfectJobs":jobUrls2, "perfectJobCount":jobUrls2.length}
}
...
async function mostCommonSubstring(testingArray1) {
    try {
        ...
        await Promise.all(pathString.map(async element => {
            if (((element.value / topValue) * 100) > 75) {
