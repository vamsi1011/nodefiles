...
async function pager() {
    try {
        const urls = await db.collection('shakerats').find({
            'status': 0
        ...
        await Promise.all(
            urls.map(async element => {
                ...
                try {
                    console.info(": Opening page: " + element.jobUrl);
                    await page.goto(element.jobUrl, {
                    ...
                    var ev_result = await page.evaluate(() => {
                        var title = document.querySelector("#main > div > div > div.grid > div.grid-item.four-fifths.lap-one-whole.palm-one-whole > h1").innerText
                        return title
                    });
                    // let cleaned_title = ev_result.split("-")[0]
                    console.log(ev_result)
                    await db.collection('shakerats').update({
                        _id: element._id
                    }, {
                        $set: {
                            "title": ev_result,
