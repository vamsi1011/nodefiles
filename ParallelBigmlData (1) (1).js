...
var BigmlAllLabels = ['http://prod-bigml-java-service-1746202486.us-east-1.elb.amazonaws.com/predict/labels']
...
async function process() {
    ...
    const dataSet = await db.collection('unique_urls_1000').find({
        'status': 2
    }).skip(200).limit(100).toArray()
            ...
            try {
                ...
                if (!responses.hasOwnProperty('error')) {
                    ...
                    for (const resp of responses) {
                        if (resp.label == 'job-title') {
                            pageLabels['Bigml_title'] = resp.value;
                        } else if (resp.label == 'job-type') {
                            pageLabels['Bigml_jobType'] = resp.value;
                        } else if (resp.label == 'expiration-date') {
                            pageLabels['Bigml_expirationDate'] = resp.value;
                        } else if (resp.label == 'job-description') {
                            pageLabels['Bigml_jobDescriptionString'] = resp.value;
                        } else if (resp.label == 'job-id') {
                            pageLabels['Bigml_jobId'] = resp.value;
                        } else if (resp.label == 'posted-date') {
                            pageLabels['Bigml_postedDate'] = resp.value;
                        } else if (resp.label == 'company') {
                            pageLabels['Bigml_company'] = resp.value;
                        } else if (resp.label == 'salary') {
                            pageLabels['Bigml_salary'] = resp.value;
                        } else if (resp.label == 'location') {
                            pageLabels['Bigml_location'] = resp.value;
                        } else {
