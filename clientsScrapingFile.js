...
// const server_uri = 'mongodb://localhost:27017/';
            ...
            case '5d446f0a7eae53350566ddae':
                ...
                if (response.length >= 1) {
                    ...
                    await db.collection(collectionName).updateMany({
                        'employerId': clientData.employerId,
                        'jobStatus': {
                            $ne: 'active'
                        }
                    }, {
...
async function emailSending(company, count, dbCount, nonDbCount, expired, collectioName) {
    if (count >= 1) {
        ...
        var mailOptions = {
            ...
            to: "mohan.pitta@jobiak.ai,hameed@jobiak.ai,sunil@jobiak.ai,nagavarma@jobiak.ai",
...
async function dataOrgJobs(page, joburl) {
    try {
        ...
        var lastPage = await page.evaluate(() => {
            let last = document.querySelector("#main > div.wrapper-joblist > div > a.last").href.match(/(\d+)\/?$/)[1]
        ...
        for (let index = 1; index <= lastPage; index++) {
            let url = `https://dataorgs.com/page/${index}/`;
...
async function dataOrgLabels(page, response) {
    try {
        ...
        for (var index = 0; index < response.length; index++) {
            const element = response[index];
            console.log("open job", index, "of", response.length, "\njobPage", element.url)
            try {
                await page.goto(element.url, {
                    networkIdle2Timeout: 90000,
                    waitUntil: "networkidle2",
