...
var pc_res = [];
var pc_api = ["http://104.251.210.66/callPageCheck",
...
function endsWithUrl(url) {
    var endwithData = ['email-alerts/', 'activate-privacy-policy.html', 'privacy-policy/', 'homepage.htm', 'services.html', '/terms-and-conditions.html', '/careers.html', 'login.xhtml', 'faq', 'careers', 'terms-of-use', 'privacy-notice', 'news', 'about-us', '/contact/', '?searchphrase=', '/staff', '/services', "/feedback", "search.html",
        "savedJobs.html", "MyChart/", "save_job/", "/searchjobs#", "/search#",
...
(async () => {
    ...
    async function CommonString(cs_arr) {
        ...
        if (pathArray.length <= 0) {
            console.log(chalk.red("no perfect jobs from page check"))
            return false
        }
        ...
        fs.appendFile("output.js", `result = {
            "url": "${testingArray[0]}",
            "allurls": "${testingArray1.length}",
            "jobs_postfilter": "${testingArray.length}",
            "time": "${Date.now() - start} secs"
        },
        `, function (err) {
    ...
    var len = testingArray.length / 10
