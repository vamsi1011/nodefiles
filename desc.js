...
async function process() {
    ...
    const Data = await db.collection('manualVsBigml(10thweek)').find({
        'expiredJobCheckStatus': 400,
        'descStatus': 200
    ...
    await Promise.all(
        ...
        Data.map(async element => {
            try {
                ...
                let PlainText = element.PlainText
                ...
                if (!DiffBot.hasOwnProperty('DiffBotHTML') || !DiffBot.hasOwnProperty('DiffBotPlainText')) {
                    ...
                    if (!DiffBot.hasOwnProperty('DiffBotHTML') && !DiffBot.hasOwnProperty('DiffBotPlainText')) {
                        ...
                        await db.collection('manualVsBigml(10thweek)').updateOne({
                            '_id': element._id
                        }, {
                            $set: DiffBot
                ...
                if (DiffBot.Diffstatus == 200) {
                    ...
                    var jobDesc = await getBigMLLabel(start, 'Description', joburl, Html, DiffBot.DiffBotPlainText, 'no_model', 'http://prod-bigml-python-service-331630396.us-east-1.elb.amazonaws.com/predict/label/jobdescription/');
                    console.log(jobDesc);
                    ...
                    var jobDesc = await getBigMLLabel(start, 'Description', joburl, Html, PlainText, 'region', 'http://prod-bigml-python-service-331630396.us-east-1.elb.amazonaws.com/predict/label/jobdescription/');
                    var pageLabels = {},
                        Description = '';
                    if (jobDesc.hasOwnProperty('value')) {
                        let Dataset = jobDesc.value
