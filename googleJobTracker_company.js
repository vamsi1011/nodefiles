    ...
    .then(async b => {
        ...
        MongoClient.connect(server_uri, {
            ...
            http
                .createServer(app)
                .listen(HTTP_PORT, () =>
                    console.info("Run Sample Using ==> http://localhost:" + HTTP_PORT + "?skip=0&limit=1&companyStatus=1")
                );
        });
    });
app.get("/", async (req, res) => {
    ...
    const Data = await db.collection('SR_unique_TL').find({
        'jobUrl': 'https://jobs.smartrecruiters.com/1-800-Flowers/85706450-purchasing-cost-assistant'
    ...
    if (Data.length) {
        ...
        try {
            ...
            const preloadFile = fse.readFileSync('./preload.js', 'utf8');
            ...
            for (let index = 0; index < Data.length; index++) {
                ...
                var googleUrl = await formGoogleUrl(title, location, company, 1);
                ...
                if (OriginalURL.indexOf("https://www.google.com/sorry/index?continue=") >= 0) {
                    ...
                    console.log("-------------------------------------------------------------------");
                    // await db.collection('SR_unique_TL').updateOne({
                    //     '_id': element._id
                    // }, {
                    //     $set: {
                    //         "blockedStatus": 1
...
async function formGoogleUrl(title, location, company, statusBit) {
    ...
    if (statusBit != 1) {
        ...
        return "https://www.google.com/search?q=" + tempTitle + "+" + tempLocation + "+&ibp=htl;jobs#fpstate=tldetail&htivrt=jobs"
    }
    return "https://www.google.com/search?q=" + tempTitle + "+" + tempLocation + '+' + tempCompany + "+&ibp=htl;jobs#fpstate=tldetail&htivrt=jobs"
}
