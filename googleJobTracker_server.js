    ...
    .then(async b => {
        ...
        MongoClient.connect(server_uri, {
            ...
            http
                .createServer(app)
                .listen(HTTP_PORT, () =>
                    console.info("Run Sample Using ==> http://localhost:" + HTTP_PORT + "?joburl=")
                );
        });
    });
...
app.get("/", async (req, res) => {
    ...
    let joburl = parseInt(req.query.joburl)    
    var htmlpt = await apiRequestFuntionForHTML(joburl);
    console.log(htmlpt)
       async function apiRequestFuntionForHTML(joburl) {
           console.log('into htmlpt')
            try {
    ...
    //             await db.collection('IcimsJan7th_ats').updateOne({
